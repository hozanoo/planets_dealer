        ...
        try:
            ...
            self.cursor.execute("""
                CREATE TABLE systems (
                    system_key TEXT PRIMARY KEY, constellation_en TEXT, distance_pc REAL,
                    hz_conservative_inner_au REAL, hz_conservative_outer_au REAL,
                    hz_optimistic_inner_au REAL, hz_optimistic_outer_au REAL
                );""")
            self.cursor.execute("""
                CREATE TABLE stars (
                    star_name TEXT PRIMARY KEY, system_key TEXT REFERENCES systems(system_key) ON DELETE SET NULL,
                    st_teff REAL, st_lum REAL, st_age REAL, st_met REAL
                );""")
            self.cursor.execute("""
                CREATE TABLE planets (
                    pl_name TEXT PRIMARY KEY, star_name TEXT REFERENCES stars(star_name) ON DELETE SET NULL,
                    ...
                    planet_type planet_type_enum, discovery_year INTEGER, orbital_period_days REAL,
                    orbit_semi_major_axis_au REAL, planet_radius_earth_radii REAL, planet_mass_earth_masses REAL,
                    orbit_eccentricity REAL, equilibrium_temperature_k REAL, insolation_flux_earth_flux REAL, esi REAL,
